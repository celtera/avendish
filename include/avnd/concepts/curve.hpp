...
template <typename T>
concept curve_segment_map_gamma = requires(T t) {
                                t.gamma = 0.f;
                              } || requires(T t) {
                                         t.power= 0.f;
                                       } ;
...
concept curve_segment_map_function= requires(T t) {
                                       t.function = [foo = 123] (auto x) { return foo * x; };
                                  } || requires(T t) {
                                       t.map = [foo = 123] (auto x) { return foo * x; };
                                       } ;
...
template <typename T>
concept curve = requires(T t) { { t[0] } -> curve_segment; };
