...
template <typename FP, typename T>
concept mono_array_sample_port = std::same_as<decltype(T::channel), FP*> || std::
    same_as<decltype(T::channel), const FP*>;
...
template <typename FP, typename T>
concept poly_array_sample_port = std::same_as<decltype(T::samples), FP**> || std::
    same_as<decltype(T::samples), const FP**>;
...
struct is_poly_array_sample_port_q
{
  template <typename T>
  using fn = std::
      conditional_t<poly_array_sample_port<FP, T>, std::true_type, std::false_type>;
};
