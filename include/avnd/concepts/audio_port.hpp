...
template <typename FP, typename T>
concept mono_array_sample_port = std::same_as<decltype(T::channel), FP*> || std::same_as<
    decltype(T::channel), const FP*>;
...
template <typename FP, typename T>
concept poly_array_sample_port = std::same_as<
    decltype(T::samples), FP**> || std::same_as<decltype(T::samples), const FP**>;
...
concept mono_audio_port = audio_sample_port<float, T> || audio_sample_port<
    double, T> || mono_array_sample_port<float, T> || mono_array_sample_port<double, T>;
...
template <typename T>
concept variable_poly_audio_port = dynamic_poly_audio_port<T> && requires(T t)
{
  t.request_channels = {};
};
