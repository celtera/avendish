...
// P2996 message introspection: discovers member functions tagged with
// [[=halp::message]] / [[=halp::message_named(...)]] and exposes them through
// the same interface (stateless_message + field_reflection) as classic messages.
...
{
  // Annotation types may be cv-qualified; strip before comparing.
...
struct reflected_message
{
  ...
  {
    // Default name is the identifier; [[=halp::message_named(...)]] overrides it.
...
// Mirrors fields_introspection's surface (size + for_all) for annotated messages.
template <typename T>
