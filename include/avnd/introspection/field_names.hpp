...
// Unified field-name accessor: prefers an explicit field_names() (the
// halp_field_names(...) macro) and otherwise, on a reflecting compiler, derives
// the names from the struct's members. This lets a struct expose field names
// with zero boilerplate while staying fully backward compatible.
