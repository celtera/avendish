...
// C++26 static-reflection backend for avnd::function_reflection<auto f>.
// A single definition replaces the ~30 cv/ref/noexcept specializations of the
// classic backend: it recovers the function (or member-function) declaration
// from the pointer value and queries it through std::meta.
...
namespace detail
{
// Class of a pointer-to-member, with a single match: a member function pointer
// R(C::*)(Args...) cv ref noexcept IS `T C::*` with T the (abominable) function
// type, so all cv/ref/noexcept qualifiers collapse into T.
template <typename M>
...
consteval std::meta::info function_decl()
{
  ...
  if constexpr(std::is_member_function_pointer_v<PT>)
  {
                     ...
                     std::meta::members_of(^^C, std::meta::access_context::unchecked())))
    {
      if constexpr(std::meta::is_function(m)
        ...
        // Guard the comparison: only same-typed overloads are comparable.
  ...
  {
    // Free function pointer: *f is an lvalue of function type.
...
// Carries class_type only for member functions (matching the classic backend,
// whose free-function specialization has no class_type).
