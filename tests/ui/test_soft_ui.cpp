...
TEST_CASE("hidpi: fractional live re-scale repaints the whole surface", "[soft_ui]")
{
  avnd::effect_container<examples::helpers::Ui> effect;
  avnd::soft_ui::surface<examples::helpers::Ui> ui{effect, test_fonts()};
  ui.resize(320, 240, 1.);
  auto fb1 = ui.frame();
  REQUIRE(fb1.width == 320);
  REQUIRE(fb1.height == 240);

  // A host announcing a fractional content scale mid-session: the next
  // frame is larger and every edge band of the new surface must hold
  // freshly painted theme background, not stale or zeroed memory (the
  // renderer skips the full clear; this is where a partial-clear bug
  // would show as an edge sliver).
  ui.resize(320, 240, 1.25);
  auto fb = ui.frame();
  REQUIRE(fb.width == 400);
  REQUIRE(fb.height == 300);

  const auto band_painted = [&](int x0, int y0, int x1, int y1) {
    int painted = 0, total = 0;
    for(int y = y0; y < y1; y++)
      for(int x = x0; x < x1; x++)
      {
        const unsigned char* px = fb.data + 4 * (y * fb.width + x);
        painted += (px[0] | px[1] | px[2]) != 0;
        total++;
      }
    // The Nuklear theme background is a non-black grey everywhere.
    return painted == total;
  };
  CHECK(band_painted(fb.width - 3, 0, fb.width, fb.height));  // right edge
  CHECK(band_painted(0, fb.height - 3, fb.width, fb.height)); // bottom edge
  CHECK(band_painted(0, 0, 3, fb.height));                    // left edge
  CHECK(band_painted(0, 0, fb.width, 3));                     // top edge

  write_ppm("soft_ui_hidpi_fractional.ppm", fb);
}
