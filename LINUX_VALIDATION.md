...
1. Fix whatever the Linux round finds (X11 blit/embed/IRunLoop most likely).
2. X11-capable UI host tests (the Win32 ones don't port; consider Xvfb in CI).
3. Perf: dirty-rect readback during drags (`get_image_data`/`putImageData`
   subrects; needs Nuklear command-buffer diffing via
   `NK_ZERO_COMMAND_MEMORY` + memcmp), glyph/label cache (~4 ms/frame of
   text re-tessellation).
