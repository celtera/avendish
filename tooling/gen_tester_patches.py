...
def gen_td_runner(dumps):
    ...
    for name, dump in dumps:
        ...
        specs.append({"name": md.get("c_name", name),
                      ...
                      "family": "TOP" if is_top else "CHOP", "pars": pars})
    return (
        "# Run inside TouchDesigner (an Execute DAT, or Text DAT > Run Script).\n"
        "# Creates each avendish Custom OP, cooks it, records errors/crashes; writes\n"
        "# td_test_report.json. TD's create() wants the OP's Python *class* (e.g.\n"
        "# AvndadditionCHOP = sanitize_td_name(c_name) + family), NOT the opType\n"
        "# string -- resolve it from the global namespace, trying likely families.\n"
        "import json\n"
        f"SPECS = {json.dumps(specs, indent=2)}\n"
        "FAMILIES = ['CHOP', 'TOP', 'SOP', 'POP', 'DAT', 'MAT']\n"
        "# A real COMP is needed as the parent (root '/' won't hold leaf ops).\n"
        "container = op('/').create(baseCOMP, 'avnd_sweep')\n"
        "report = []\n"
        "RP = project.folder + '/td_test_report.json'\n"
        "CUR = project.folder + '/td_current.txt'\n"
        "# Resume: load any prior report so a relaunch (after a crash) skips what\n"
        "# was already tested -- including the crasher, which the harness records.\n"
        "try:\n"
        ...
        "# Everything is inlined (no helper functions): this script is exec()'d\n"
        "# inside an Execute DAT callback, where a nested def can't see these\n"
        "# module-level names. eval() resolves the Custom OP class (a TD builtin).\n"
        "for s in SPECS:\n"
        "    if s['name'] in done: continue  # already tested (resume after crash)\n"
        "    # Breadcrumb: if an op hard-crashes TD, td_current.txt names it and\n"
        "    # td_test_report.json holds everyone before it.\n"
        "    open(CUR, 'w').write(s['name'])\n"
        "    cls = None\n"
        "    fams = []\n"
        ...
        "            n.cook(force=True)\n"
        "            report.append({'name': s['name'], 'ok': not n.errors(),\n"
        "                           'errors': n.errors(), 'warnings': n.warnings()})\n"
        "            n.destroy()\n"
        "        except Exception as e:\n"
        "            report.append({'name': s['name'], 'ok': False, 'exception': str(e)})\n"
