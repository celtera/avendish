...
def gen_td_runner(dumps):
    ...
    return (
        ...
        "        # Pull the output through a downstream Null CHOP: in a batch, TD\n"
        "        # only queries getOutputInfo/execute when something pulls the op's\n"
        "        # output, so an unconnected output-less op (a value-IO object with\n"
        "        # no input) would otherwise report 0 channels. Reading the Null's\n"
        "        # channels forces the cook.\n"
        "        cap = n\n"
        "        try:\n"
        "            sink = container.create(nullCHOP, 'sink_' + s['name'])\n"
        "            if len(sink.inputConnectors) > 0:\n"
        "                sink.inputConnectors[0].connect(n)\n"
        "                sink.cook(force=True)\n"
        "                cap = sink\n"
        "        except Exception:\n"
        "            cap = n\n"
        "        try:\n"
        "            entry['dbg_nc'] = cap.numChans\n"
        "            entry['dbg_ns'] = cap.numSamples\n"
        "        except Exception: pass\n"
        "        td_out = []\n"
        "        try:\n"
        "            ns = cap.numSamples\n"
        ...
        "        except Exception:\n"
        "            entry['td_out'] = None\n"
        "        # Control outputs that aren't signal channels (val_port on an\n"
        "        # object with no CHOP output) surface only via getInfoCHOPChan --\n"
        "        # read them through an Info CHOP (generic info channels are\n"
