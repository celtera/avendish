...
def gen_td_runner(dumps):
    ...
    for name, dump in dumps:
        ...
        specs.append({"name": md.get("c_name", name),
                      "type": sanitize_td_name(md.get("name", name)),
                      "family": "TOP" if is_top else "CHOP", "pars": pars})
    return (
        "# Run inside TouchDesigner (Text DAT > Run Script, or as a startup DAT).\n"
        "# Creates each avendish Custom OP, cooks it, records errors; writes report.\n"
        "# 'type' is the registered opType; adjust create() to your TD if needed.\n"
        "import json, td\n"
        ...
        "for s in SPECS:\n"
        "    try:\n"
        "        n = root.create(s['type'], 'test_' + s['name'])\n"
        ...
        "open(project.folder + '/td_test_report.json', 'w').write(json.dumps(report, indent=2))\n"
