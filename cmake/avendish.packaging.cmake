...
# --- Pure Data (addon) ------------------------------------------------------
#
# avnd_create_pd_addon_package(
#   NAME <addon-name> SOURCE_PATH <dir> PACKAGE_ROOT <dir>
#   EXTERNALS <target> ...      # the *_pd external targets
#   [SUPPORT <file> ...])
#
# Pd resolves an external and its <name>-help.pd from the same flat folder, so
# the package is a single directory: <root>/<name>/<external><suffix> +
# <name>-help.pd + metadata. Matches the layout the ossia/actions pd packager and
# the Deken composite step expect (per-OS folders union file-by-file).
function(avnd_create_pd_addon_package)
  cmake_parse_arguments(AVND "" "NAME;SOURCE_PATH;PACKAGE_ROOT" "EXTERNALS;SUPPORT" ${ARGN})
  if(NOT AVND_EXTERNALS)
    return()
  endif()
  ...
  foreach(_external ${AVND_EXTERNALS})
    add_custom_command(TARGET ${_external} POST_BUILD
      ...
      COMMAND ${CMAKE_COMMAND} -E copy "$<TARGET_FILE:${_external}>" "${_pkg}/"
      COMMENT "Packaging Pd external: ${_external}"
      VERBATIM)

    get_target_property(_pd_help ${_external} AVND_PD_HELP)
    if(_pd_help)
      # Interactive help patch (avnd_generate_help). Best-effort: a missing help
      # patch (some toolchains) must not fail packaging.
      if(TARGET "${_external}_help")
        add_dependencies("${_external}" "${_external}_help")
      endif()
      get_property(_pkgdir GLOBAL PROPERTY AVND_PACKAGING_DIR)
  ...
  # SUPPORT libs sit next to the externals (Pd's loader searches the external's
  # own folder for siblings).
  foreach(_support ${AVND_SUPPORT})
    add_custom_command(TARGET ${_pkg_target} POST_BUILD
      COMMAND ${CMAKE_COMMAND} -E make_directory "${_pkg}"
      COMMAND ${CMAKE_COMMAND} -E copy_if_different "${_support}" "${_pkg}/"
      VERBATIM)
  endforeach()
endfunction()
...
# --- Python (addon) ---------------------------------------------------------
#
# avnd_create_python_addon_package(
#   NAME <addon-name> SOURCE_PATH <dir> PACKAGE_ROOT <dir>
#   EXTERNALS <target> ...      # the *_python module targets (py<c_name>.so/.pyd)
#   [SUPPORT <file> ...])
#
# Layout: <root>/<name>/py<c_name>.<so|pyd> + <root>/<name>/__init__.py (re-exports
# every compiled port) + <root>/<name>/examples/<c_name>_example.py + metadata.
# Mirrors the ossia/actions python packager (which synthesizes the same __init__).
function(avnd_create_python_addon_package)
  cmake_parse_arguments(AVND "" "NAME;SOURCE_PATH;PACKAGE_ROOT" "EXTERNALS;SUPPORT" ${ARGN})
  if(NOT AVND_EXTERNALS)
    return()
  endif()
  ...
  set(_pkg "${AVND_PACKAGE_ROOT}/${AVND_NAME}")
  set(_pkg_target "${AVND_NAME}_python_package")
  add_custom_target(${_pkg_target} ALL DEPENDS ${AVND_EXTERNALS})
  ...
  # Loader shim: re-export every py<port> module as an attribute of the package,
  # tolerating a single failed import. Addon-independent, written once.
  file(WRITE "${_pkg}/__init__.py"
"# Auto-generated by avendish (avnd_create_python_addon_package).
# Re-exports every compiled avendish port (py<port>) as a package attribute.
import importlib as _importlib
import glob as _glob
import os as _os

_here = _os.path.dirname(__file__)
for _f in _glob.glob(_os.path.join(_here, 'py*.*')):
    _ext = _os.path.splitext(_f)[1].lower()
    if _ext not in ('.so', '.pyd'):
        continue
    _name = _os.path.splitext(_os.path.basename(_f))[0]
    try:
        globals()[_name] = _importlib.import_module('.' + _name, package=__name__)
    except Exception as _exc:
        import sys as _sys
        print('avendish: failed to import {}: {}'.format(_name, _exc), file=_sys.stderr)

del _importlib, _glob, _os
")
  ...
  foreach(_external ${AVND_EXTERNALS})
    add_custom_command(TARGET ${_external} POST_BUILD
      COMMAND ${CMAKE_COMMAND} -E make_directory "${_pkg}"
      COMMAND ${CMAKE_COMMAND} -E copy "$<TARGET_FILE:${_external}>" "${_pkg}/"
      COMMENT "Packaging Python module: ${_external}"
      VERBATIM)

    get_target_property(_py_example ${_external} AVND_PYTHON_EXAMPLE)
    if(_py_example)
      if(TARGET "${_external}_help")
        add_dependencies("${_external}" "${_external}_help")
      endif()
      ...
      add_custom_command(TARGET ${_external} POST_BUILD
        COMMAND ${CMAKE_COMMAND}
          "-DSRC=${CMAKE_BINARY_DIR}/${_py_example}" "-DDST=${_pkg}/examples"
          -P "${_pkgdir}/avendish.packaging.copy_optional.cmake"
        VERBATIM)
    endif()
  endforeach()
...
function(avnd_addon_package)
  ...
  # Mark that an addon has assembled its packages explicitly, so the default
  # auto-packaging in avnd_addon_finalize() stands down (an explicit call is how
  # addons pass SUPPORT libs / a custom PACKAGE_ROOT; auto-packaging again would
  # duplicate POST_BUILD copies and miss the SUPPORT).
  set_property(GLOBAL PROPERTY AVND_ADDON_PACKAGE_INVOKED 1)
  ...
  foreach(_backend ${AVND_BACKENDS})
    ...
    elseif(_backend STREQUAL "pd")
      avnd_create_pd_addon_package(
        ...
        EXTERNALS ${_externals}
        SUPPORT ${AVND_SUPPORT})
    elseif(_backend STREQUAL "python")
      avnd_create_python_addon_package(
        NAME "${AVND_NAME}"
        SOURCE_PATH "${AVND_SOURCE_PATH}"
        PACKAGE_ROOT "${AVND_PACKAGE_ROOT}"
